...
format[0].addEventListener('click',()=>{
    item[0].setAttribute('src',"../img/shop/cassette1.jpg");
    item[1].setAttribute('src',"../img/shop/cassette2.jpg");
    item[2].setAttribute('src',"../img/shop/cassette3.jpg");
    reset();
});
format[1].addEventListener('click',()=>{
    item[0].setAttribute('src',"../img/shop/vinyl1.jpg"); //https://unsplash.com/@brett_jordan?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText
    item[1].setAttribute('src',"../img/shop/vinyl2.jpg"); //https://unsplash.com/pt-br/@brett_jordan?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText
    item[2].setAttribute('src',"../img/shop/vinyl3.jpg"); //https://unsplash.com/@brett_jordan?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText
    reset();
});
format[2].addEventListener('click',()=>{
    item[0].setAttribute('src',"../img/shop/cd1.jpg"); //https://www.freepik.com/free-vector/electro-music-album_3601855.htm#query=album%20cover&position=39&from_view=keyword&track=ais
    item[1].setAttribute('src',"../img/shop/cd2.jpg"); //https://www.freepik.com/free-vector/electro-music-album_3601853.htm#query=album%20cover&position=34&from_view=keyword&track=ais
    item[2].setAttribute('src',"../img/shop/cd3.jpg"); //https://www.freepik.com/free-psd/cd-cover-editable-template-psd-dark-tone-corporate-identity_18983286.htm#query=album%20cover&position=29&from_view=keyword&track=ais
    reset();
});
...
function cart_update(i,item_price){
    ...
    price[i].innerHTML = "Rs."+price_int+".00";

    //Calculate the total price
    total[i] += price_int;
    total_price += total[i];
    document.getElementById("total-price").innerHTML = "Rs."+total_price+".00";
    button_activation();
}
